 ...
 * @author iiley
 */
class org.aswing.tree.DefaultMutableTreeNode implements MutableTreeNode{
     ...
     */
    public function DefaultMutableTreeNode(userObject:Object, allowsChildren:Boolean) {
		parent = null;
		if(allowsChildren == undefined) allowsChildren = true;
    ...
    public function removeAllChildren():Void {
		for (var i:Number = getChildCount()-1; i >= 0; i--) {
		    removeAt(i);
		}
    }
     ...
     *         specified node 
     */
    private function getPathToRoot(aNode:TreeNode, depth:Number):Array {
		var retNodes:Array;
    ...
    public function getChildAfter(aChild:TreeNode):TreeNode {
		...
		var index:Number = getIndex(aChild);		// linear search
    ...
    public function getLeafCount():Number {
		...
		if (count < 1) {
			trace("Error : tree has zero leaves");
