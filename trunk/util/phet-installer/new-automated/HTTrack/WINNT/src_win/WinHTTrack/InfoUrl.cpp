...
void CInfoUrl::OnTimer(UINT nIDEvent) 
{
  ...
  if (back) {
    ...
    if (back[id].status != -1) {        // utilisé
      if (back[id].r.totalsize>0) {
        sprintf(total,"%d",back[id].r.totalsize);
        ...
        if (back[id].status != -1) {        // utilisé
          ...
          if (backitem->location_buffer)
            ...
            if (backitem->is_chunk)
              sprintf(moreinfo+strlen(moreinfo),"CurrentChunkSize: %d\r\n",backitem->chunk_size);
            //
            ...
            //
            sprintf(moreinfo+strlen(moreinfo),"Size: %d\r\n",backitem->r.size);
            sprintf(moreinfo+strlen(moreinfo),"TotalSize: %d\r\n",backitem->r.totalsize);
        } else
          strcpybuff(moreinfo,"Transfer complete in this buffer, waiting for next file");
      }
      //
      SetDlgItemTextCP(this, IDC_InfoUrl,info);
      SetDlgItemTextCP(this, IDC_Info100,info100);
      SetDlgItemTextCP(this, IDC_MoreInfoUrl,moreinfo);
      //
