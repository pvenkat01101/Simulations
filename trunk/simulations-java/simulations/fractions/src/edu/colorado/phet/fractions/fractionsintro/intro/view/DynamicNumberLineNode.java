 ...
 * @author Sam Reid
 */
public class DynamicNumberLineNode extends PNode {
    ...
    public DynamicNumberLineNode( final ObservableProperty<Integer> numerator, final SettableProperty<Integer> settableNumerator, final ObservableProperty<Integer> denominator, ValueEquals<Representation> showing, final IntegerProperty max, final double distanceBetweenTicks, Color color, final boolean arrowPathVisible,
        ...
        final RichSimpleObserver mainObserver = new RichSimpleObserver() {
            @Override public void update() {
                ...
                int rightDivisionsPerTick = denominator.get();
                int leftDivisionsPerTick = leftDenominator.get();

                double rightDX = distanceBetweenTicks / rightDivisionsPerTick;
                double leftDX = distanceBetweenTicks / leftDivisionsPerTick;

                //The number line itself
                addChild( new PhetPPath( orientation.line( 0, 0, rightDX * max.get() * rightDivisionsPerTick, 0 ) ) );

                //For snapping
                tickLocations = new ArrayList<Pair<Double, Integer>>();

                final BasicStroke highlightStroke = new BasicStroke( 4 );
                for ( int i = 0; i <= rightDivisionsPerTick * max.get(); i++ ) {

                    //Major ticks at each integer
                    if ( i % rightDivisionsPerTick == 0 ) {
                        int div = i / rightDivisionsPerTick;
                        final int mod = div % 2;
                        double height = mod == 0 ? 8 : 8;
                        final BasicStroke stroke = mod == 0 ? new BasicStroke( 1 ) : new BasicStroke( 0.5f );
                        final PhetPPath path = new PhetPPath( orientation.line( i * rightDX, -height, i * rightDX, height ), stroke, Color.black );

                        final boolean highlightVisible = numerator.get().equals( i );
                        if ( highlightVisible ) {
                            final PhetPPath highlightPath = new PhetPPath( orientation.line( i * rightDX, -height, i * rightDX, height ), highlightStroke, Color.yellow );
                        ...
                        tickLocations.add( new Pair<Double, Integer>( i * rightDX, i ) );
                    }
                }
                ...
                for ( int i = 0; i <= rightDivisionsPerTick * max.get(); i++ ) {
                    if ( !( i % rightDivisionsPerTick == 0 ) ) {
                        ...
                        final boolean highlightVisible = numerator.get().equals( i );
                        if ( highlightVisible ) {
                            final PhetPPath highlightPath = new PhetPPath( orientation.line( i * rightDX, 0, i * rightDX, 4 ), highlightStroke, Color.yellow );
                            highlights.add( highlightPath );
                            addChild( highlightPath );
                        }

                        //minor ticks between the integers
                        addChild( new PhetPPath( orientation.line( i * rightDX, 0, i * rightDX, 4 ), new BasicStroke( 0.25f ), Color.black ) );
                ...
                //Left ticks
                for ( int i = 0; i <= leftDivisionsPerTick * max.get(); i++ ) {
                    if ( !( i % leftDivisionsPerTick == 0 ) ) {

                        double tickValue = ( (double) i ) / leftDivisionsPerTick;
                        double fractionValue = ( (double) numerator.get() ) / denominator.get();
                        boolean highlightVisible = Math.abs( fractionValue - tickValue ) < 1E-6;
                        if ( highlightVisible ) {
                            final PhetPPath highlightPath = new PhetPPath( orientation.line( i * leftDX, -4, i * leftDX, 0 ), highlightStroke, Color.yellow );
                            highlights.add( highlightPath );
                            addChild( highlightPath );
                        }

                        //minor ticks between the integers
                        addChild( new PhetPPath( orientation.line( i * leftDX, -4, i * leftDX, 0 ), new BasicStroke( 0.25f ), Color.black ) );

                        //make it so the green handle can snap to this site
                        tickLocations.add( new Pair<Double, Integer>( i * leftDX, i ) );
                    }
                }
    ...
    private static GeneralPath createArrowShape( final double scale, final ArrayList<PNode> highlights ) {
        Rectangle2D highlightBounds = highlights.get( 0 ).getFullBounds();
        for ( PNode highlight : iterableList( highlights ).tail() ) {
            highlightBounds = highlightBounds.createUnion( highlight.getFullBounds() );
        }
