...
abstract class MotionSeriesCanvas(model: MotionSeriesModel,
        ...
        extends PlayArea(800, modelViewport) {
  //max y should be about 10 in default case
  ...
  if ( useVectorNodeInPlayArea ) {
    vectorView.addAllVectorsAllComponents(model.motionSeriesObject, playAreaVectorNode)
  }
