...
class Airborne(private var _position2D: Vector2D,
  ...
  override def stepInTime(dt: Double) = {
    ...
    if ( _position2D.y <= motionSeriesObject.airborneFloor ) {
      //Crashed
...
class Grounded(motionSeriesObject: MotionSeriesObject) extends MotionStrategy(motionSeriesObject) {
  ...
  override def wallForce = {
    ...
    if ( pressing ) {
      ...
      val finalVelocity = if ( bounce ) {
        -motionSeriesObject.velocity
      }
      else {
        0.0
      }
  ...
  override def frictionForce(includeWallForce: Boolean) = {
    if ( motionSeriesObject.surfaceFriction() ) {
      ...
      if ( motionSeriesObject.velocity.abs < 1E-12 ) {
        ...
        val netForceWithoutFriction = motionSeriesObject.appliedForce.value + motionSeriesObject.gravityForce.value + normalForce + ( if ( includeWallForce ) {
          wallForce
        }
        else {
          new Vector2D()
        ...
        val magnitude = if ( netForceWithoutFriction.magnitude >= fMax ) {
          fMax
        }
        else {
          netForceWithoutFriction.magnitude
        }
  ...
  def getNewState(dt: Double, origState: MotionSeriesObjectState, origEnergy: Double) = {
    val newVelocity = {
      ...
      val newVelocityThatGoesThroughZero = if ( ( motionSeriesObject.velocity < 0 && desiredVel > 0 ) || ( motionSeriesObject.velocity > 0 && desiredVel < 0 ) ) {
        0.0
      }
      else {
        desiredVel
      ...
      if ( collide && bounce ) {
        ...
        newVelocityThatGoesThroughZero
      }
    }
    ...
    val stateAfterFixingPosition = if ( abs(stateAfterFixingVelocity.totalEnergy - origEnergy) > 1E-8 && getAngle > 1E-8 ) {
      //todo: angle is greater than 1E-8 instead of 0 for compatibility with workaround for forces and motion game tab, see ForcesAndMotionApplication
    ...
    if ( delta.abs > 1E-6 && appliedEnergy.abs < 1E-4 ) {
      //assume applied energy could absorb some error
    ...
    if ( collide && bounce ) {
      motionSeriesObject.notifyBounced()
    }
    else if ( collide && !bounce ) {
      motionSeriesObject.notifyCollidedWithWall()
    }
