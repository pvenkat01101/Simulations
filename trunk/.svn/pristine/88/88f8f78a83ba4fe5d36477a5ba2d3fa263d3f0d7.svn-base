...
public class RealMoleculeShape {
    ...
    private void translate( Vector3D offset ) {
    ...
    public static final RealMoleculeShape BERYLLIUM_CHLORIDE = new RealMoleculeShape( "BeCl2", 1.8 ) {{ // TODO: more accurate numbers?
        addCentralAtom( new Atom3D( Be, new Vector3D() ) );
        addRadialAtom( new Atom3D( Cl, new Vector3D( 1.8, 0, 0 ), 3 ), 1 );
        addRadialAtom( new Atom3D( Cl, new Vector3D( -1.8, 0, 0 ), 3 ), 1 );
    }};

    public static final RealMoleculeShape BORON_TRIFLUORIDE = new RealMoleculeShape( "BF3", 1.313 ) {{
        addCentralAtom( new Atom3D( B, new Vector3D() ) );
        double angle = 2 * Math.PI / 3;
        double bondLength = 1.313;
        addRadialAtom( new Atom3D( F, new Vector3D( bondLength * Math.cos( 0 * angle ), bondLength * Math.sin( 0 * angle ), 0 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( bondLength * Math.cos( 1 * angle ), bondLength * Math.sin( 1 * angle ), 0 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( bondLength * Math.cos( 2 * angle ), bondLength * Math.sin( 2 * angle ), 0 ), 3 ), 1 );
    }};

    public static final RealMoleculeShape BROMINE_PENTAFLUORIDE = new RealMoleculeShape( "BrF5", 1.774 ) {{
        addCentralAtom( new Atom3D( B, new Vector3D(), 1 ) );
        ...
        addRadialAtom( new Atom3D( F, new Vector3D( 0, -axialBondLength, 0 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( radialDistance, -axialDistance, 0 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( 0, -axialDistance, radialDistance ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( -radialDistance, -axialDistance, 0 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( 0, -axialDistance, -radialDistance ), 3 ), 1 );
    }};

    public static final RealMoleculeShape METHANE = new RealMoleculeShape( "CH4", 1.087 ) {{
        addCentralAtom( new Atom3D( C, new Vector3D() ) );
        double bondLength = 1.087;
        for ( Vector3D v : GeometryConfiguration.getConfiguration( 4 ).unitVectors ) {
    ...
    public static final RealMoleculeShape CHLORINE_TRIFLUORIDE = new RealMoleculeShape( "ClF3", 1.698 ) {{
        addCentralAtom( new Atom3D( Cl, new Vector3D(), 2 ) );
        addRadialAtom( new Atom3D( F, new Vector3D( 0, -1.598, 0 ), 3 ), 1 );
    ...
    public static final RealMoleculeShape CARBON_DIOXIDE = new RealMoleculeShape( "CO2", 1.163 ) {{
        addCentralAtom( new Atom3D( C, new Vector3D() ) );
        addRadialAtom( new Atom3D( O, new Vector3D( -1.163, 0, 0 ), 2 ), 2 );
        addRadialAtom( new Atom3D( O, new Vector3D( 1.163, 0, 0 ), 2 ), 2 );
    }};

    public static final RealMoleculeShape WATER = new RealMoleculeShape( "H2O", 0.957 ) {{
        addCentralAtom( new Atom3D( O, new Vector3D(), 2 ) );
        double radialBondLength = 0.957;
        double radialAngle = Math.toRadians( 104.5 ) / 2;
        addRadialAtom( new Atom3D( H, new Vector3D( Math.sin( radialAngle ), -Math.cos( radialAngle ), 0 ).times( radialBondLength ) ), 1 );
        addRadialAtom( new Atom3D( H, new Vector3D( -( Math.sin( radialAngle ) ), -Math.cos( radialAngle ), 0 ).times( radialBondLength ) ), 1 );
    }};

    public static final RealMoleculeShape AMMONIA = new RealMoleculeShape( "NH3", 1.017 ) {{
        addCentralAtom( new Atom3D( N, new Vector3D(), 1 ) );
        ...
        addRadialAtom( new Atom3D( H, new Vector3D( radialDistance * Math.cos( 0 * radialAngle ), -axialDistance, radialDistance * Math.sin( 0 * radialAngle ) ) ), 1 );
        addRadialAtom( new Atom3D( H, new Vector3D( radialDistance * Math.cos( 1 * radialAngle ), -axialDistance, radialDistance * Math.sin( 1 * radialAngle ) ) ), 1 );
        addRadialAtom( new Atom3D( H, new Vector3D( radialDistance * Math.cos( 2 * radialAngle ), -axialDistance, radialDistance * Math.sin( 2 * radialAngle ) ) ), 1 );
    }};

    public static final RealMoleculeShape PHOSPHORUS_PENTACHLORIDE = new RealMoleculeShape( "PCl5", 2.02 ) {{
        addCentralAtom( new Atom3D( P, new Vector3D() ) );
        addRadialAtom( new Atom3D( Cl, new Vector3D( 2.14, 0, 0 ), 3 ), 1 );
        addRadialAtom( new Atom3D( Cl, new Vector3D( -2.14, 0, 0 ), 3 ), 1 );
        double radialAngle = 2 * Math.PI / 3;
        double radialBondLength = 2.02;
        addRadialAtom( new Atom3D( Cl, new Vector3D( 0, Math.cos( 0 * radialAngle ), Math.sin( 0 * radialAngle ) ).times( radialBondLength ), 3 ), 1 );
        addRadialAtom( new Atom3D( Cl, new Vector3D( 0, Math.cos( 1 * radialAngle ), Math.sin( 1 * radialAngle ) ).times( radialBondLength ), 3 ), 1 );
        addRadialAtom( new Atom3D( Cl, new Vector3D( 0, Math.cos( 2 * radialAngle ), Math.sin( 2 * radialAngle ) ).times( radialBondLength ), 3 ), 1 );
    }};

    public static final RealMoleculeShape SULFUR_TETRAFLUORIDE = new RealMoleculeShape( "SF4", 1.595 ) {{
        addCentralAtom( new Atom3D( S, new Vector3D(), 1 ) );
        double largeAngle = Math.toRadians( 173.1 ) / 2;
        double smallAngle = Math.toRadians( 101.6 ) / 2;

        addRadialAtom( new Atom3D( F, new Vector3D( Math.sin( largeAngle ), -Math.cos( largeAngle ), 0 ).times( 1.646 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( -Math.sin( largeAngle ), -Math.cos( largeAngle ), 0 ).times( 1.646 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( 0, -Math.cos( smallAngle ), Math.sin( smallAngle ) ).times( 1.545 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( 0, -Math.cos( smallAngle ), -Math.sin( smallAngle ) ).times( 1.545 ), 3 ), 1 );
    }};

    public static final RealMoleculeShape SULFUR_HEXAFLUORIDE = new RealMoleculeShape( "SF6", 1.564 ) {{
        addCentralAtom( new Atom3D( S, new Vector3D() ) );
        for ( Vector3D vector : GeometryConfiguration.getConfiguration( 6 ).unitVectors ) {
    ...
    public static final RealMoleculeShape SULFUR_DIOXIDE = new RealMoleculeShape( "SO2", 1.431 ) {{
        ...
        addCentralAtom( new Atom3D( S, new Vector3D(), 1 ) );
        addRadialAtom( new Atom3D( O, new Vector3D( Math.sin( bondAngle ), -Math.cos( bondAngle ), 0 ).times( bondLength ), 2 ), 2 );
        addRadialAtom( new Atom3D( O, new Vector3D( -Math.sin( bondAngle ), -Math.cos( bondAngle ), 0 ).times( bondLength ), 2 ), 2 );
    }};

    public static final RealMoleculeShape XENON_DIFLUORIDE = new RealMoleculeShape( "XeF2", 1.977 ) {{
        addCentralAtom( new Atom3D( Xe, new Vector3D(), 3 ) );
        addRadialAtom( new Atom3D( F, new Vector3D( 1.977, 0, 0 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( -1.977, 0, 0 ), 3 ), 1 );
    }};

    public static final RealMoleculeShape XENON_TETRAFLUORIDE = new RealMoleculeShape( "XeF4", 1.953 ) {{
        double bondLength = 1.953;
        addCentralAtom( new Atom3D( Xe, new Vector3D(), 2 ) );
        addRadialAtom( new Atom3D( F, new Vector3D( bondLength, 0, 0 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( -bondLength, 0, 0 ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( 0, 0, bondLength ), 3 ), 1 );
        addRadialAtom( new Atom3D( F, new Vector3D( 0, 0, -bondLength ), 3 ), 1 );
    }};
