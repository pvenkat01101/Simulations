...
(function () {
     ...
     */
    AttractorModel.applyAttractorForces = function ( groups, timeElapsed, idealOrientations, allowablePermutations, center, angleRepulsion ) {
        var currentOrientations = phet.util.map( groups, function ( group ) {
            return group.position.get().minus( center ).normalized();
        } );
        var mapping = AttractorModel.findClosestMatchingConfiguration( currentOrientations, idealOrientations, allowablePermutations );
...
//                pair.position.set( targetLocation );
     ...
     */
    AttractorModel.findClosestMatchingConfiguration = function ( currentOrientations, idealOrientations, allowablePermutations ) {
        ...
        for ( var pIndex = 0; pIndex < allowablePermutations.length; pIndex++ ) {
            ...
            if ( bestResult == null || error < bestResult.error ) {
                bestResult = new AttractorModel.ResultMapping( error, target, permutation, rot );
            }
        }
        return bestResult;
    };
