    ...
    if (SInfo.refresh) {
      ...
      if (!icn) {
        ...
        if (SInfo.stat_time>0) {
          ...
          SetDlgItemTextCP(inprogress, IDC_i1 ,s);  // time
        } else
          SetDlgItemTextCP(inprogress, IDC_i1 , _SN(SInfo.stat_time) );  // time
        SetDlgItemTextCP(inprogress, IDC_i0 , int2bytes(&strc, SInfo.stat_bytes) );  // bytes
        SetDlgItemTextCP(inprogress, IDC_i2 , lnk);  // scanned
        if (SInfo.stat_nsocket>0)
          SetDlgItemTextCP(inprogress, IDC_i3 , _SN(SInfo.stat_nsocket) );  // socks
        else
          SetDlgItemTextCP(inprogress, IDC_i3 , "none" );  // wait
        
        CString st;
        st.Format("%s (%s)", int2bytessec(&strc, SInfo.irate), int2bytessec(&strc2, SInfo.rate));
        SetDlgItemTextCP(inprogress, IDC_i4 , st );  // rate
        
        SetDlgItemTextCP(inprogress, IDC_i5 , _SN(SInfo.stat_errors) );
        SetDlgItemTextCP(inprogress, IDC_i6 , _SN(SInfo.stat_written) );
        {
          char tempo[256];
          int pc=0;
          ...
          SetDlgItemTextCP(inprogress, IDC_i7 , tempo );
        }
        ...
        /*if (!parsing)*/
        {
          ...
          // redraw en boucle
          {
            ...
            for( ; i<NStatsBuffer;i++) {
              ...
              st.Replace("&", "&&");
              SetWindowTextCP(inprogress->element[1][i], st);
              st = StatsBuffer[i].fichier;
              st.Replace("&", "&&");
              SetWindowTextCP(inprogress->element[4][i], st);
        ...
        if (parsing) {  // parsing
          ...
          SetWindowTextCP(inprogress->element[1][0], StatsBuffer[0].nom);
...
void lance(void) {
  ...
  {
    char *a;
    while(a=strchr(ShellOptions->LINE,9)) *a=' ';
    while(a=strchr(ShellOptions->LINE,10)) *a=' ';
    while(a=strchr(ShellOptions->LINE,13)) *a=' ';
  }
