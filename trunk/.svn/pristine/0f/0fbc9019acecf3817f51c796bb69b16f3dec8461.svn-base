// Copyright 2002-2011, University of Colorado
package edu.colorado.phet.chemicalreactions;

import edu.colorado.phet.common.phetcommon.resources.PhetResources;

/**
 * Resources (images and translated strings) for "ChemicalReactions" are loaded eagerly to make sure everything exists on sim startup, see #2967.
 * Automatically generated by edu.colorado.phet.buildtools.preprocessor.ResourceGenerator
 */
public class ChemicalReactionsResources {
    public static final String PROJECT_NAME = "chemical-reactions";
    public static final PhetResources RESOURCES = new PhetResources( PROJECT_NAME );

    //Strings
    public static class Strings {
        public static final String BUCKET___CH_4 = RESOURCES.getLocalizedString( "bucket.CH4" );
        public static final String BUCKET___CO_2 = RESOURCES.getLocalizedString( "bucket.CO2" );
        public static final String BUCKET___CL_2 = RESOURCES.getLocalizedString( "bucket.Cl2" );
        public static final String BUCKET___CL_NO = RESOURCES.getLocalizedString( "bucket.ClNO" );
        public static final String BUCKET___CL_NO_2 = RESOURCES.getLocalizedString( "bucket.ClNO2" );
        public static final String BUCKET___H_2 = RESOURCES.getLocalizedString( "bucket.H2" );
        public static final String BUCKET___H_2_O = RESOURCES.getLocalizedString( "bucket.H2O" );
        public static final String BUCKET___H_CL = RESOURCES.getLocalizedString( "bucket.HCl" );
        public static final String BUCKET___N_2 = RESOURCES.getLocalizedString( "bucket.N2" );
        public static final String BUCKET___NH_3 = RESOURCES.getLocalizedString( "bucket.NH3" );
        public static final String BUCKET___NO = RESOURCES.getLocalizedString( "bucket.NO" );
        public static final String BUCKET___NO_2 = RESOURCES.getLocalizedString( "bucket.NO2" );
        public static final String BUCKET___O_2 = RESOURCES.getLocalizedString( "bucket.O2" );
        public static final String KIT__LABEL = RESOURCES.getLocalizedString( "kit.label" );
        public static final String KIT__RESET_KIT = RESOURCES.getLocalizedString( "kit.resetKit" );
        public static final String TITLE__CHEMICAL_REACTIONS = RESOURCES.getLocalizedString( "title.chemicalReactions" );
    }

    //Images
    public static class Images {

    }
}