...
public class StripChart extends JPanel {
    ...
    protected void paintComponent( Graphics g ) {
        ...
        g2.drawLine( plotAreaULC.x, plotAreaULC.y + ( height / 2 ), plotAreaULC.x + width, plotAreaULC.y + ( height / 2 ) );
        ...
        for ( int i = 1; i < data.length; i++ ) {
            ...
            if ( ( i % tickSpace ) == tickLoc ) {
                g2.setColor( tickColor );
                g2.drawLine( i + plotAreaULC.x, plotAreaULC.y, i + plotAreaULC.x, plotAreaULC.y + height );
            }

            // Draw data point
            g2.setColor( Color.RED );
            g2.drawLine( i - 1 + plotAreaULC.x, d0 + plotAreaULC.y - yOffset, i + plotAreaULC.x, d1 + plotAreaULC.y - yOffset );
        }
    }
