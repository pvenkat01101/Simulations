...
public enum MoleculeShapesColor {
    ...
    public static ColorHandler<MoleculeShapesColor> handler = new ColorHandler<MoleculeShapesColor>() {{
        setProfile( DEFAULT );
    }};
    ...
    public void addLWJGLColorObserver( final VoidFunction1<Color> callback ) {
        getProperty().addObserver( LWJGLUtils.jmeObserver( new Runnable() {
            public void run() {
                callback.apply( get() );
            }
        } ) );
