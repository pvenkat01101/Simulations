...
/**
 * Class that depicts a slider that controls a parameter of type Double in a
...
class LogarithmicParameterSliderNode extends PNode {
    ...
    LogarithmicParameterSliderNode( IUserComponent userComponent, final double min, final double max, final SettableProperty<Double> settableProperty, String htmlLabelText ) {
        ...
        HSliderNode sliderNode = new HSliderNode( userComponent, sliderMin, sliderMax, SLIDER_TRACK_THICKNESS,
                                                  SLIDER_TRACK_LENGTH, passThroughProperty, new BooleanProperty( true ) ) {{
        ...
        passThroughProperty.addObserver( new VoidFunction1<Double>() {
            public void apply( Double sliderValue ) {
                settableProperty.set( MathUtil.clamp( min, Math.pow( 10, sliderValue ), max ) );
            }
        } );
