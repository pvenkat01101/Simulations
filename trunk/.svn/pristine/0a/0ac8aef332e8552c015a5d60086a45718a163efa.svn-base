...
public class AlphaDecayEnergyChart extends PNode implements SubatomicParticle.Listener {
    ...
    private final ArrayList _currentlyTrackedAlphas = new ArrayList( MAX_ALPHA_PARTICLES_DISPLAYED );
    ...
    private final PImage[] _alphaParticleImages = new PImage[MAX_ALPHA_PARTICLES_DISPLAYED];
    ...
    private void update() {
        ...
        if ( _usableWidth <= 0 || _usableHeight <= 0 ) {
            // Ignore this.  It sometimes happens at startup.
            return;
        }
    ...
    private class GraphLegend extends PNode {
        ...
        private final PPath _background;
        ...
        public void updateSize( double width, double height ) {
            ...
            _title.setScale( 1 );
            double titleScale = 1;
            titleScale = ( width * 0.9 ) / _title.getFullBoundsReference().width;
            if ( _title.getFullBoundsReference().height > height / 4 ) {
                titleScale = Math.min( ( height / 4 ) / _title.getFullBoundsReference().height, titleScale );
            }
            _title.setScale( titleScale );
            ...
            _totalEnergyLabel.setScale( 1 );
            if ( _totalEnergyLabel.getFullBoundsReference().height > height / 4 ) {
                _totalEnergyLabel.setScale( ( height / 4 ) / _totalEnergyLabel.getFullBoundsReference().height );
            }
            ...
            _potentialEnergyLabel.setScale( 1 );
            if ( _potentialEnergyLabel.getFullBoundsReference().height > height / 4 ) {
                _potentialEnergyLabel.setScale( ( height / 4 ) / _potentialEnergyLabel.getFullBoundsReference().height );
            }
