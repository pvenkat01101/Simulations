 ...
 * @author iiley
 */
class org.aswing.tree.AbstractLayoutCache implements RowMapper{
    ...
    public function getPreferredHeight():Number {
		...
		if(rowCount > 0) {
		    var bounds:Rectangle = getBounds(getPathForRow(rowCount - 1), null);
     ...
     */
    public function getPreferredWidth(bounds:Rectangle):Number {
		var rowCount:Number = getRowCount();
		if(rowCount > 0) {
		    // Get the width
		    ...
		    var paths:Array = getVisiblePathsFrom(firstPath, Number.MAX_VALUE);
			var pn:Number = paths.length;
		    if(paths != null && pn>0){
				var pBounds:Rectangle = getBounds(TreePath(paths[0]), null);
				var width:Number;
				...
				while (pBounds != null && i<pn) {
				    pBounds = getBounds(TreePath(paths[i]), pBounds);
    ...
    public function getRowsForPaths(paths:Array):Array {
		...
		var numPaths:Number = paths.length;
		var rows:Array = new Array(numPaths);
	
		for(var counter:Number = 0; counter < numPaths; counter++)
     ...
     */
    private function countNodeDimensions(value:Object, row:Number, depth:Number,
					  expanded:Boolean, placeIn:Rectangle):Rectangle {
